      ...
      <div v-if="baseData.length > 0" class="informList">
        <div v-for="(item, index) in baseData" :key="index">
          ...
          <div v-if="item.type === 2" class="item">
            <div class="tit">
              <i>急</i><span>【待履约接单】</span>{{ item.arrNew[0]
          ...
          >
            <div :class="isActive ? 'titAlready' : ''">
              ...
              <div v-if="shopShow && showIndex === index" class="orderInfo">
                ...
                <p>
                  <span><label>商品：</label>{{ item.details.orderDishes }}</span>
                </p>
              </div>
            </div>
          </div>
          ...
          >
            <div :class="isActive ? 'titAlready' : ''">
              ...
              <div v-if="shopShow && showIndex === index" class="orderInfo">
                <p>
                  <span><label>GMV：</label>{{ item.details.turnover }}</span>
...
export default class extends Vue {
  ...
  private notificationApiAvailable = true
  private notificationApiWarned = false
  ...
  /**
   * 判断消息中心接口是否为后端缺口，缺口时页面走空态降级。
   */
  private isNotificationApiMissing(error: any) {
    const status = error && error.response ? error.response.status : undefined
    return status === 404 || status === 405
  }

  /**
   * 统一处理消息中心接口缺口，避免页面重复弹错。
   */
  private handleNotificationApiMissing() {
    this.notificationApiAvailable = false
    this.baseData = []
    this.counts = 0
    AppModule.StatusNumber(0)
    if (!this.notificationApiWarned) {
      this.notificationApiWarned = true
      this.$message.warning('当前通知中心接口尚未完成后端接入，已降级为空状态显示')
    }
  }
  ...
  async getData() {
    ...
    try {
      const { data } = await getNotificationPage(parent)
      if (data.code === 1) {
      ...
      if (this.isNotificationApiMissing(error)) {
        this.handleNotificationApiMissing()
        return
      }
  ...
  async handleBatch() {
    ...
    try {
      const { data } = await batchReadNotification(ids)
      if (data.code === 1) {
        this.getCountUnread()
        this.getData()
      } else {
