<template>
  <div class="container">
    ...
    <div class="orderviewBox">
      <ul>
        <li>
          <span class="status"><i class="iconfont icon-waiting" />待履约接单</span>
          <span class="num tip"><router-link to="/order?status=2">{{
            orderviewData.waitingOrders
          }}</router-link></span>
        </li>
        <li>
          <span class="status"><i class="iconfont icon-staySway" />待履约配送</span>
...
import { WorkspaceOrderOverview } from '@/api/workspace'
@Component({
  name: 'Orderview',
})
export default class extends Vue {
  @Prop() private orderviewData!: WorkspaceOrderOverview
  get days() {
    return getday()
  }
}
