    ...
    <div class="container">
      <div class="retail-guide">
        ...
        <div class="retail-guide__tags">
          <el-tag v-for="item in retailCategoryGuide"
                  :key="item"
                  size="small"
          >
            {{ item }}
      ...
      >
        ...
        <div style="float: right">
          <el-button v-permission="'product:category:add'"
                     type="primary"
                     class="continue"
      ...
      >
        ...
        >
          <template slot-scope="scope">
            <span>{{ scope.row.type == '1' ? '商品分类' : '组合商品分类' }}</span>
          </template>
        </el-table-column>
        ...
        >
          <template slot-scope="scope">
            <el-button v-permission="'product:category:edit'"
                       type="text"
                       size="small"
                       class="blueBug"
            ...
            <el-button v-permission="'product:category:delete'"
                       type="text"
                       size="small"
                       class="delBut"
            ...
            <el-button v-permission="'product:category:status'"
                       type="text"
                       size="small"
                       class="non"
                       :class="{
...
export default class extends Vue {
  ...
  // 固定展示社区零售推荐分类，帮助新增分类时保持后台和小程序一致
  private retailCategoryGuide: string[] = [
    '新鲜蔬菜',
    '时令水果',
    '肉禽蛋品',
    '粮油调味',
      ...
      name: [
        {
          ...
          validator: (rule: any, value: string, callback: Function) => {
            ...
            } else if (trimValue.length < 2 || trimValue.length > 20) {
              callback(new Error('分类名称输入不符，请输入2-20个字符'))
            } else if (!reg.test(trimValue)) {
              callback(new Error('分类名称只能包含中文或英文字母'))
            } else {
              callback()
            }
          }
  ...
  private statusHandle(row: any) {
    ...
    this.$confirm('确认调整该分类的状态?', '提示', {
      ...
      updateProductCategoryStatus({ id: this.id, status: !this.status ? 1 : 0 })
        .then(res => {
