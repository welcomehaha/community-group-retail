  ...
  <div class="navbar">
    ...
    >
      <div class="rightStatus">
        ...
        <span
          v-if="hasPermission('store:status:view')"
          v-permission="'store:status:edit'"
          class="navicon operatingState"
          @click="handleStatus"
        ><i />调整营业状态</span>
      </div>
    ...
    >
      <el-radio-group v-model="setStatus">
        <el-radio :label="1">
          营业中
          <span>当前门店处于营业状态，自动接收订单，可点击打烊进入门店暂停营业状态。</span>
        </el-radio>
        <el-radio :label="0">
          打烊中
          <span>当前门店处于打烊状态，仅接受营业时间内的预定订单，可点击营业中手动恢复营业状态。</span>
        </el-radio>
      </el-radio-group>
      <span slot="footer"
            class="dialog-footer"
...
export default class extends Vue {
  ...
  webSocket() {
    ...
    const envSocketUrl = process.env.VUE_APP_SOCKET_URL || ''
    const socketBaseUrl = envSocketUrl
      ? envSocketUrl.replace(/\/?$/, '/')
      : `${window.location.protocol === 'https:' ? 'wss' : 'ws'}://${window.location.host}/ws/`
    let socketUrl = socketBaseUrl + clientId
    ...
    if (typeof WebSocket == 'undefined') {
      ...
      this.websocket.onmessage = function (msg) {
        ...
        that.$notify({
          title: jsonMsg.type === 1 ? '待履约接单' : '催履约',
          duration: 0,
      ...
      this.websocket.onerror = function () {
        console.error('WebSocket连接失败:', socketUrl)
      }
      // 监听socket关闭
  ...
  // 营业状态
  async getStatus() {
    // 无查看权限时不再请求营业状态接口，避免越权和无效请求。
    if (!this.hasPermission('store:status:view')) {
      this.status = 0
      this.setStatus = 0
      return
    }
    const { data } = await getStoreStatus()
    this.status = data.data !== null && data.data !== undefined ? data.data : 0
  ...
  // 设置营业状态
  handleStatus() {
    if (!this.hasPermission('store:status:edit')) {
      this.$message.warning('暂无调整营业状态权限')
      return
    }
  ...
  async handleSave() {
    ...
    const { data } = await updateStoreStatus(this.setStatus)
    if (data.code === 1) {
...
.navbar {
  ...
  // overflow: hidden;
  position: relative;
  background: linear-gradient(90deg, #0b2f2a 0%, #115e59 58%, #16a34a 100%);
  ...
  .closing {
    background: #e97a4c;
    box-shadow: 0 8px 16px rgba(233, 122, 76, 0.22);
  }
  .navicon {
    i {
      display: inline-block;
