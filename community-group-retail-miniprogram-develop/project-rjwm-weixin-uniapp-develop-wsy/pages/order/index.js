...
import DishInfo from "./components/dishInfo.vue" //商品信息
	...
	methods: {
		...
		handlePiker() {
			if (this.tableware !== '') {
				...
				if (this.tableware === '依据商品数量提供') {
					this.num = this.orderDishNumber
					this.status = 1
				}

				if (this.tableware !== '依据商品数量提供' || this.tableware !== '无需餐具') {
