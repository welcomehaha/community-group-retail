...
import dishDetail from "./components/dishDetail.vue" //商品详情
import {
	// 社区团购即时零售相关接口
	userLogin,
	getCategoryList,
...
export default {
	data() {
		return {
			...
			// 添加一个右侧number更新以后重新刷新接口的id --- 这个id来自左侧商品分类的id
			rightIdAndType: {},
			phoneData: "",
			tablewareNumber: 0,
			shopStatus: null,
	...
	methods: {
		...
		async addDishAction(item, form) {
			...
			if (
				...
				!this.orderListDataes.some((n) => n.productId == item.id || n.dishId == item.id) &&
			...
			if (item.productSpec !== "" && item.productSpec) {
				productSpecData = item.productSpec
			} else if (item.dishFlavor !== "" && item.dishFlavor) {
				productSpecData = item.dishFlavor
			} else if (flavorRemark.length > 0) {
				productSpecData = flavorRemark.join(',')
			} else {
				productSpecData = null
			}
			let params = {
				productSpec: productSpecData,
			}
			if (item.type === 1) {
				params = {
					...params,
				...
				if (item.productId || item.dishId) {
					...
					params = {
						productBundleId: item.productBundleId || item.setmealId,
					}
				}
			}
		...
		async redDishAction(item, form) {
			...
			if (item.productSpec !== "" && item.productSpec) {
				...
				productSpecData = flavorRemark[0]
			} else {
				productSpecData = null
			}
			let params = {
					...
					if (CData.length > 0) {
						CData &&
							CData.forEach((tg, ind) => {
								const productId = tg.productId || tg.dishId
								const productBundleId = tg.productBundleId || tg.setmealId
								if (obj.id === productId || obj.id === productBundleId) {
