  ...
  <view>
    ...
    <view class="my-center">
      ...
      <view class="container">
        <!-- 地址、订单和在线客服入口 -->
        <order-info @goAddress="goAddress" @goOrder="goOrder" @goCustomerAi="goCustomerAi"></order-info>
        <!-- 最近选购 -->
        <!-- 最近选购title -->
        <view
          class="recent"
        ...
        >
          <text class="order_line">最近选购</text>
        </view>
        <order-list
          :scrollH="scrollH"
          @lower="lower"
...
export default {
  ...
  methods: {
    ...
    // 跳转到 AI 在线客服页面
    goCustomerAi(orderInfo) {
      const orderNumber = orderInfo && (orderInfo.number || orderInfo.orderNumber);
      const url = orderNumber
        ? `/pages/customerAi/index?orderNumber=${encodeURIComponent(orderNumber)}&intent=order_progress`
        : "/pages/customerAi/index";
      uni.navigateTo({ url });
    },
